...
pub trait PrimitiveLike {
    ...
    ) -> Option<Box<dyn PrimitiveContext + 'static>> {
        None
    }
}

pub fn retrieve_context<T: PrimitiveContext + 'static>(
    exec_state: &mut ExecutionState,
    args: &[core_relations::Value],
) -> Option<(T, Vec<core_relations::Value>)> {
    let (last_arg, args) = args.split_last().unwrap();
    let PrimitiveContextValue(_, _, value) = exec_state.prims().unwrap(*last_arg);
    if let Some(value) = value.into_any().downcast::<T>().ok() {
        Some((*value, args.to_vec()))
    } else {
        None
    }
}
...
impl Default for EGraph {
    fn default() -> Self {
        ...
        eg.backend
            .primitives_mut()
            .register_type::<PrimitiveContextValue>();
        eg
    }
}
...
pub trait PrimitiveContext: DynClone + Send + Sync {
    fn into_any(self: Box<Self>) -> Box<dyn Any>;
}
dyn_clone::clone_trait_object!(PrimitiveContext);

/// Stores some context associated with a primitive generated by the type checking so that it can be used during invocation.
/// Hashing, Equality, and Debug are all implemented based on the typed terms and function ID instead of the context, assuming that creating
/// the context is idempotent based on the types.
#[derive(Clone)]
struct PrimitiveContextValue(
    ExternalFunctionId,
    Vec<core::ResolvedAtomTerm>,
    Box<dyn PrimitiveContext + 'static>,
);

impl PartialEq for PrimitiveContextValue {
    fn eq(&self, other: &Self) -> bool {
        self.0 == other.0
            && self.1.len() == other.1.len()
            && self.1.iter().zip(&other.1).all(|(a, b)| a == b)
    }
}

impl Eq for PrimitiveContextValue {}

impl Hash for PrimitiveContextValue {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.0.hash(state);
...
impl Debug for PrimitiveContextValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "PrimitiveContextValue({:?}, {:?})", self.0, self.1)
    }
}
...
impl<'a> BackendRule<'a> {
    ...
    fn prim(
        ...
        {
            let value = PrimitiveContextValue(prim.primitive.1, args.to_vec(), context);
            qe_args.push(self.rb.egraph().primitive_constant(value));
        };
        (
            prim.primitive.1,
            qe_args,
